...
RETCODE sm4_cbc_mode_decrypt(unsigned char* key, unsigned char* iv,
  ...
  if (sm4_cbc_decrypt_update(&cbc_ctx, ciphertext, cipher_len, plaintext, &outlen) != 1) {
    fprintf(stderr, "%s %d: error\n", __FILE__, __LINE__);
    return RETCODE_FAIL;
  }
  *plain_len = outlen;
...
int main(void)
{
  ...
  constexpr size_t plaintext_len = 500 * 1024 * 1024ll;

  std::cout << "plain text len: " << plaintext_len << "\n";
  ...
  std::cout << "prepare data time cost(ms): " << time_cost << "\n";
  {
    ...
    std::cout << "encrypt data time cost(ms): " << time_cost << "\n";
  }

  std::cout << "inlen: " << inlen << " outlen: " << outlen << "\n";
  ...
  _end = std::chrono::high_resolution_clock::now();
  time_cost = std::chrono::duration_cast<std::chrono::milliseconds>(_end - _start).count();
  std::cout << "decrypt data time cost(ms): " << time_cost << "\n";
  std::cout << "inlen: " << inlen << " outlen: " << outlen << "\n";
  // std::string rec_str{reinterpret_cast<char*>(rec_buf), inlen};
  outbuf_ptr.reset();
  std::this_thread::sleep_for(std::chrono::seconds(5));
  if (memcmp(inbuf, rec_buf, plaintext_len) == 0) {
    std::cout << "equal with origin data\n";
  } else {
    std::cout << "not equal with orgin data\n";
  }
	return 0;
}
